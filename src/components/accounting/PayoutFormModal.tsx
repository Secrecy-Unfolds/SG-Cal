...
import type { InvestmentRow } from "@/lib/investors";
...
export default function PayoutFormModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/investments/${investment.id}/payouts`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount: amountNum, currency, date }),
      });
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center px-4 z-50">
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="font-heading font-semibold text-lg uppercase tracking-wide">Record payout</h2>
          <button
            type="button"
            onClick={onClose}
