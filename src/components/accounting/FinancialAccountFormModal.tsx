...
export default function FinancialAccountFormModal({
  account,
  onClose,
  onSaved,
}: {
  ...
  const [accountType, setAccountType] = useState<FinancialAccountType>(account?.account_type ?? "bank");
  const [currency, setCurrency] = useState(account?.currency ?? "OMR");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res = await fetch(account ? `/api/financial-accounts/${account.id}` : "/api/financial-accounts", {
        method: account ? "PATCH" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name.trim(), accountType, currency }),
      });
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center px-4 z-50">
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="font-heading font-semibold text-lg uppercase tracking-wide">{account ? "Edit account" : "New account"}</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <select className={inputClass} value={accountType} onChange={(e) => setAccountType(e.target.value as FinancialAccountType)}>
