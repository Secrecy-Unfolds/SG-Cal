...
import { deleteRecurringExpense, updateRecurringExpense } from "@/lib/recurringExpenses";
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!isAdminLevel(session.role)) {
    return NextResponse.json({ error: "Only Admins and Super Admins can edit recurring expenses" }, { status: 403 });
  }
  ...
  const recurring = await updateRecurringExpense(id, { description, category, amount, currency, frequency, nextRunDate, active });
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!isAdminLevel(session.role)) {
    return NextResponse.json({ error: "Only Admins and Super Admins can delete recurring expenses" }, { status: 403 });
  }
