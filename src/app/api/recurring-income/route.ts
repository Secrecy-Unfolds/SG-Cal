...
import { createRecurringIncome, listRecurringIncome } from "@/lib/recurringIncome";
...
export async function GET() {
  ...
  const recurring = await listRecurringIncome();
...
export async function POST(req: NextRequest) {
  ...
  if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!isAdminLevel(session.role)) {
    return NextResponse.json({ error: "Only Admins and Super Admins can add recurring income" }, { status: 403 });
  }
