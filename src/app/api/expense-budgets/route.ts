...
import { createExpenseBudget, listExpenseBudgets } from "@/lib/expenseBudgets";
...
export async function GET() {
  ...
  const budgets = await listExpenseBudgets();
...
export async function POST(req: NextRequest) {
  ...
  const budget = await createExpenseBudget({ category, periodStart, periodEnd, amount, currency, createdBy: session.uid });
